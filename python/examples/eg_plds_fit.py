"""Based on matlab/test_plds_fit.m
...
dt = 1e-3
A = np.array([[0.95, -0.05], [0, 0.9]])
n_x = A.shape[0]
B = np.array([[0], [0.25]])
C = np.eye(n_x)
n_u = B.shape[1]
n_y = C.shape[0]
d = np.zeros((n_y, 1)) + np.log(0.01)  # output bias

sys = plds.System(n_u, n_x, n_y, dt)
sys.A = A
sys.B = B
sys.C = C
sys.d = d
...
np.random.seed(1820)  # for reproducibility
...
ssid = plds.FitSSID(n_x_fit, n_h, dt, u_train, z_train, d)
...
pve = 1 - np.var(y_a - y_hat_a, axis=(1, 2)) / np.var(y_a, axis=(1, 2))
...
# var over dims 1 and 2, time and trial
pve_em = 1 - np.var(y_a - y_hat_em_a, axis=(1, 2)) / np.var(y_a, axis=(1, 2))
