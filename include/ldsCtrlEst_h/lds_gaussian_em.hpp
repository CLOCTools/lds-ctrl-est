...
class emFit_t : public fit_t {
 ...
 protected:
  ...
  void kalmanFilt();
  void kalmanSmooth(
      bool forceCommonInitial);  // estimate x_t|T by kalman filtering -->
                                 // backwards filtering.
  void Estep(bool forceCommonInitial =
                 false);  // force all trials to have common initial conditions?
...
// std::tuple<emFit_t, armaVec> emFit_x_equals_y_qM(std::vector<armaMat>&
// uTrain, std::vector<armaMat>& zTrain,  std::vector<data_t>& t0, data_t dt,
// std::size_t maxIter=100, data_t tol=1e-3);
};  // namespace gaussian
}  // namespace lds
