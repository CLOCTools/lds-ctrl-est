...
int main(void) {
	...
	aArma_true[0] = exp(-dt/0.01);
	armaMat bArma_true = armaMat(nX,nU,fill::eye);
	bArma_true[0] = 4.25e-4;
	...
	// true system...
	plds::sys_t sys_true(nU, nX, nY, dt, p0, q0_true);
	...
	data_t uLB = 0.0;
	data_t uUB = 5.0;
	glds::ctrl_t sys(nU, nX, nY, uLB, uUB, dt, p0, q0, r0);
	...
	stdVec bVec = armaVec2stdVec( vectorise(sys.getB()) );
	stdVec dVec = armaVec2stdVec( vectorise(sys.getD()) );
	stdVec qVec = armaVec2stdVec( vectorise(Q) );

	stdVec x0Vec = armaVec2stdVec( x0Sys );
	...
	matfile = Mat_CreateVer("test_glds_ctrl.mat",NULL,MAT_FT_DEFAULT);
