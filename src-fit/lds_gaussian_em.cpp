...
// TODO: this is a fudge, but for some reason, cov mats often going numerically
// asymm.
...
void glds::emFit_t::reset(void) {
  ...
  // // go ahead and put g into B so I don't have to worry about messing with
  // it. B = B * diagmat(g); g.ones();
...
void glds::emFit_t::Mstep(bool calcAB, bool calcQ, bool calcInitial, bool calcC,
  ...
  armaMat phi;
  if (calcAB) {
    // AB = sum_E_xu_t_xu_tm1.submat(0,0,nX-1,nX+nU-1) *
  ...
  if (calcQ) {
    if (calcAB) {
      ...
      // Q = (sum_E_xu_t_xu_t.submat(0,0,nX-1,nX-1) -
      // AB*sum_E_xu_t_xu_tm1.submat(0,0,nX-1,nX+nU-1).t())/T;
      Q = (sum_E_xu_t_xu_t - phi * sum_E_xu_t_xu_tm1.t()) / T;
    } else {
      // Shumway, Stoffer 1982
      // Same as the above, but in case did not re-solve for AB
      // Q = ( sum_E_xu_t_xu_t.submat(0,0,nX-1,nX-1) -
      // sum_E_xu_t_xu_tm1.submat(0,0,nX-1,nX+nU-1) *
      // inv_sympd(sum_E_xu_tm1_xu_tm1) *
      // sum_E_xu_t_xu_tm1.submat(0,0,nX-1,nX+nU-1).t()) / T;
    ...
    // {
    // 	armaMat AB_k = join_horiz(AB.submat(k,k,k,k),
    // AB.submat(k,nX,k,nX+nU-1)); 	armaMat sum_E_x_t_xu_tm1_k =
    // join_horiz(sum_E_xu_t_xu_tm1.submat(k,k,k,k),
    // sum_E_xu_t_xu_tm1.submat(k,nX,k,nX+nU-1));
    //
    // 	Q.diag()[k] = (sum_E_xu_t_xu_t.submat(k,k,k,k) -
    // AB_k*sum_E_x_t_xu_tm1_k.t())[0]/T;
    // }
  ...
  if (calcInitial) {
    ...
    P0.zeros();
    for (size_t trial = 0; trial < zTrain.size(); trial++) {
      // P[trial].slice(0) = (xHat[trial].col(0)*xHat[trial].col(0).t()) +
      // P[trial].slice(0) + eVar; P0 +=  P[trial].slice(0);
...
/*
tuple<emFit_t,armaVec> glds::emFit_x_equals_y_qM(vector<armaMat>& uTrain,
vector<armaMat>& zTrain, data_t dt, size_t maxIter, data_t tol)
{
        ...
        armaMat A = join_vert(join_horiz(fit.A, armaMat(nX,nX,fill::eye)),
armaMat(nX,2*nX,fill::zeros)); A.submat(nX,nX,2*nX-1,2*nX-1) =
armaMat(nX,nX,fill::eye); armaMat B = join_vert(fit.B,
armaMat(nX,nU,fill::zeros)); armaVec m(2*nX,fill::zeros); armaMat Q =
join_vert(join_horiz(fit.Q, armaMat(nX,nX,fill::zeros)),
armaMat(nX,2*nX,fill::zeros)); Q.submat(nX,nX,2*nX-1,2*nX-1) =
1e-9*armaMat(nX,nX,fill::eye); armaVec x0 = join_vert(fit.x0, fit.m); armaMat C
= join_horiz(fit.C, armaMat(nY,nX,fill::zeros)); armaVec d(nY, fill::zeros);
        ...
        // TODO: this makes an unnecessary copy, but want the convenience of
calling constructor to make x,P,etc emFit_t fit_m(A, B, fit.g, m, Q, x0, C, d,
fit.R, dt, uTrain, zTrain); fit_m.maxIter = maxIter; fit_m.tol = tol;
