...
ssidFit_t lds::gaussian::ssidFit(vector<armaMat>& u, vector<armaMat>& z,
  ...
  for (size_t k = 0; k < z.size(); k++) {
    ...
    zSSID.submat(0, nSamps, nY - 1, nSamps + len - 1) =
        z[k].submat(0, idxStart[k], nY - 1, idxStop[k]);  /// dt;
  ...
  armaMat P0(nX, nX, fill::zeros);  // TODO: FUDGE. See VanOvershee 1996 for how
                                    // to get P0 from SSID...
  ...
  if (d0.is_finite() && (d0.n_rows == nY)) {
    ...
    // TODO: implement least-square solution for impulse response with a second
    // input of ones. For now... Calculate baseline rate (d) as the output
    // wherever the stimulus has not been on for some amount of time. convolve u
    // with rectangle and take all samples
  ...
  // ssid_n4sid_vanOverschee(nX, nU, nY, nH, uSSID, zSSID, A, B, C, D, Q, R, s,
  // wt);
...
void lds::gaussian::ssid_n4sid_vanOverschee(size_t& nX, size_t& nU, size_t& nY,
  ...
  switch (wt) {
    ...
    case MOESP: {
      ...
      armaMat O_k_ortho_Uf_sans_Qt =
          join_horiz(Ob_subL - (Ob_subL * pinv(Ru)) * Ru, Ob_subR);  // good

      // armaMat Pi = armaMat(2*nH*nU,2*nH*nU,fill::eye) - R_23_13.t() *
      // inv(R_23_13*R_23_13.t()) * R_23_13; armaMat O_k_ortho_Uf_sans_Qt =
  ...
  // // Instead, going to use alternative in van Overschee 1996, p. 129.
  // Apparently, should ensure stability. C =
  // extObs_k.submat(0,0,nY-1,extObs_k.n_cols-1); armaMat extObs_kp1 =
  // join_vert(extObs_k.submat(nY,0,extObs_k.n_rows-1,extObs_k.n_cols-1),
  // armaMat(nY,extObs_k.n_cols,fill::zeros)); A = pinv(extObs_k) * extObs_kp1;

  A = S.submat(0, 0, nX - 1, nX - 1);
  C = S.submat(nX, 0, nX + nY - 1, nX - 1);

  // At this point, van Overschee & de Moor suggest re-calculating extObs_k,
  // extObs_km1 from (A, C) because it was just an approximation.
...
void lds::gaussian::ssid_n4sid(size_t& nX, size_t& nU, size_t& nY, size_t& nH,
  ...
  // Going to use alternative in van Overschee 1996, p. 129. Apparently, should
  // ensure stability.
  ...
  if (doConstrainDB) {
    ...
    // Going to make weight on minimizing DC error immense so at least that
    // should be nailed.
