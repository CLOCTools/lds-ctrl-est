...
tuple<ssidFit_t, lds::gaussian::ssidFit_t> lds::poisson::ssidFit(
  ...
  // Bc of how fitOutput_mle() is written, it does not preserve the y=x
  // structure, so no point in allowing this right now.
  // TODO: rewrite fitOutput_mle() to only re-scale C rather than refit each
  // element. When this happens, will make the force_unitNormC option visible
  ...
  // iteratively refit d, C by Poisson MLE given x (as if there is zero process
  // noise)
    ...
    for (size_t l = 0; l <= itersAllowed; l++) {
      ...
      for (size_t k = 0; k < x.size(); k++) {          // trial loop
        for (size_t kk = 0; kk < x[0].n_cols; kk++) {  // time samp loop
          // f += exp(d[p] + c_p*x[k].col(kk))[0]*dt * x[k].col(kk) -
          // n[k].col(kk)[p] * x[k].col(kk); fprime += exp(d[p] +
          // c_p*x[k].col(kk))[0]*dt * x[k].col(kk) * x[k].col(kk).t();
      ...
      // f_over_fprime *= 0.1;//0.05;//0.1;//optionally could as a step size
      // here to slow down if numerical issues
