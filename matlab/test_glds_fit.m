%%%
%%% This script fits a GLDS model to simulated data using subspace identification,
%%% then refines the fit using expectation-maximization.
%%%
...
n_x_fit = n_x;
tic()
sing_vals = glds_ssid_mex(sys_hat, u, z, dt, n_x_fit, 25, d, false, 1, 0);
toc()
...
if (do_refit)
  ...
  calcC = true; %calculate output (C)
  calcd = true; %calculate output bias (d)
  calcR = true; %calculate output noise (R)
  n_iter = 100;
  tol = 1e-3;
  ...
  glds_em_refit_mex(sys_hat, u, z, ...
    calcAB, calcQ, calcInitial, ...
    calcC, calcd, calcR, ...
    n_iter, tol ...
    );
